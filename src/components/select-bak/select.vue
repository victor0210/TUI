<template>
  <label class="t-select" :class="{
    ...
    <transition name="fade">
      <t-select-drop-menu v-if="initialized" :isFocus="isFocus" :parent="this">
        <!--<template slot-scope="scope">-->
          <!--<t-option slot="search" :label="scope.select-bak.getEditContent" :val="scope.select-bak.editContent" :editablePanel="true" v-if="scope.select-bak.editable || scope.select-bak.searchable" :disabled="scope.select-bak.searchable" v-show="scope.select-bak.editable || (scope.select-bak.searchable && scope.select-bak.optionChildren.length === 1)" :loading="scope.select-bak.loading"/>-->
        <!--</template>-->
      </t-select-drop-menu>
    </transition>
  </label>
...
import TSelectDropMenu from './select-drop-menu.vue'
import Vue from 'vue'

//  TODO rebuild select-bak component data-structure from array to index (build store-index)
...
export default {
  ...
  mounted () {
    this.self = this
    this.initStoreIndex()
    this.$on('select-bak-drop-register', this.dropRegister)
  ...
  methods: {
    ...
    checkout (e) {
      ...
      if (!this.isFocus) {
        ...
        if (this.isFocus && !this.initialized) this.initialized = true
